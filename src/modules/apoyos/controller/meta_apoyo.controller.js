const MetaApoyoService = require("../service/meta_apoyo.service");
...
const logger = rootLogger.child({ module: "meta_apoyo.controller" });
...
// Reglas de negocio específicas de meta_apoyo:
// - Validación de creador obligatorio
// - Validación de título obligatorio (máx 160 caracteres)
// - Validación de descripción obligatoria (máx 2000 caracteres)
// - Auditoría de creación de metas
async function create(req, res) {
  const startedAt = Date.now();
  const eventName = "meta_apoyo_create";

  try {
    sendAttemptingRequest(req, logger, eventName, { body: req.body });
    const result = await MetaApoyoService.create(req.body);
    return sendServiceResponse(req, res, eventName, startedAt, result, 201);
  } catch (error) {
    sendServerInternalError(req, logger, eventName, startedAt, "meta_apoyo.controller.create", error);
...
async function update(req, res) {
  const startedAt = Date.now();
  const eventName = "meta_apoyo_update";

  try {
    sendAttemptingRequest(req, logger, eventName, { params: req.params, body: req.body });
    const result = await MetaApoyoService.update(req.params, req.body);
    return sendServiceResponse(req, res, eventName, startedAt, result);
  } catch (error) {
    sendServerInternalError(req, logger, eventName, startedAt, "meta_apoyo.controller.update", error);
...
async function get(req, res) {
  const startedAt = Date.now();
  const eventName = "meta_apoyo_get";

  try {
    sendAttemptingRequest(req, logger, eventName, { params: req.params });
    const result = await MetaApoyoService.get(req.params);
    return sendServiceResponse(req, res, eventName, startedAt, result);
  } catch (error) {
    sendServerInternalError(req, logger, eventName, startedAt, "meta_apoyo.controller.get", error);
...
async function list(req, res) {
  const startedAt = Date.now();
  const eventName = "meta_apoyo_list";

  try {
    sendAttemptingRequest(req, logger, eventName, { query: req.query });
    const result = await MetaApoyoService.list(req.query);
    return sendServiceResponse(req, res, eventName, startedAt, result);
  } catch (error) {
    sendServerInternalError(req, logger, eventName, startedAt, "meta_apoyo.controller.list", error);
